...
│   ├── ReviewOutput.tsx                       ← Преглед (property + GL + OA)
│   ├── DownloadPDFButton.tsx                  ← PDF download (property + GL + OA)
│   ├── SectionSidebar.tsx
│   ├── FieldRenderer.tsx
│   └── pdf/
...
| Застраховател | Формуляр           | PDF компонент      | Уникални секции                                     |
|---------------|--------------------|--------------------|-----------------------------------------------------|
| Алианц        | VAPROSNIK_0142     | AllianzOAPDF       | Оценка на риска (аварии 10г., злополуки 3г., смени, концентрация), доброволен лимит за ВН |
| Групама       | Групама-Злополука  | GroupamaOAPDF      | Пакет А/Б, 8 допълнителни покрития, битова злополука, период ВН (>11/>21/>31 дни) |

**OA Schema секции:**
- `oa_applicant` — Данни за застраховащия (8 полета, EIK auto-fill)
- `oa_insurance` — Данни за застраховката (7 полета)
- `oa_coverage` — Покрити рискове и суми (10 полета)
- `oa_options` — Допълнителни покрития (9 полета, само Групама)
- `oa_risk` — Оценка на риска (11 полета, само Алианц)

**Бизнес правила:**
- Задължителна з-ка (Наредба ПМС 24/2006): з.с. = 7 × годишна брутна заплата
- Групама Пакет А: база = общ фонд р.з. ИЛИ една МБЗ на лице
- Допълнителните покрития на Групама важат за Пакет А и Пакет Б
